...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(res.user?.profileComplete ? '/app' : '/onboarding');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      ...
      <motion.div
        className="w-full max-w-md glass rounded-2xl p-8 border border-violet-500/20"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-violet-300 mb-1">Password (min 8 chars)</label>
            <input
              type="password"
              value={password}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 rounded-lg bg-gradient-to-r from-neon-pink to-neon-blue text-white font-semibold hover:opacity-90 transition disabled:opacity-50"
          >
        ...
        <div className="mt-6">
          <button
            ...
            className="w-full py-3 rounded-lg glass border border-violet-500/30 hover:border-violet-400/50 transition flex items-center justify-center gap-2"
          >
